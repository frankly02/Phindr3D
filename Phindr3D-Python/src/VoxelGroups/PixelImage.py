        ...
        for zplane in slices:
            ...
            for jChan in range(metadata.GetNumChannels()):
                if metadata.intensityNormPerTreatment:
                    ...
                    croppedIM[:, :, jChan] = DataFunctions.rescaleIntensity(io.imread(img, '.tif'), low=metadata.lowerbound[jChan], high=metadata.upperbound[jChan])
                else:
                    img = image.stackLayers[zplane].channels[jChan + 1].channelpath
                    croppedIM[:, :, jChan] = DataFunctions.rescaleIntensity(io.imread(img, '.tif'), low=metadata.lowerbound[jChan], high=metadata.upperbound[jChan])
            ...
            if croppedIM.shape[0] >= training.pixelsPerImage:
                arr = np.array([croppedIM[i,:] for i in metadata.Generator.choice(croppedIM.shape[0],size=training.pixelsPerImage, replace=False, shuffle=False)])
                trPixels[startVal:startVal + training.pixelsPerImage, :] = arr
